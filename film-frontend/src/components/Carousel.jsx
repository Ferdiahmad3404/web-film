...
const Carousel = () => {
  ...
  return (
    <div id="default-carousel" className="relative w-full h-screen" data-carousel="slide">
      ...
      {/* Slider indicators */}
      <div className="absolute z-20 flex -translate-x-1/2 space-x-3 bottom-5 left-1/2">
        {films.map((_, index) => (
          <button
            key={index}
            type="button"
      ...
      {/* Slider controls */}
      <button type="button" className="absolute top-0 start-0 z-60 flex items-center justify-center h-full px-4 cursor-pointer" onClick={prevSlide}>
      ...
      <button type="button" className="absolute top-0 end-0 z-60 flex items-center justify-center h-full px-4 cursor-pointer" onClick={nextSlide}>
        <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30">
          <svg className="w-4 h-4 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
